...
def generate_launch_description():
    ...
    state_joint = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_potrol_description, 'launch', 'state_joint_publisers_ackermann.launch.py'),
        )
    )

    controller_spawner = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_potrol_bringup, 'launch', 'controller.launch.py'),
        )
    )

    # robot_name_arg = DeclareLaunchArgument('robot_name', default_value='potrol')
    # x_spawn_arg = DeclareLaunchArgument('x_spawn', default_value='-88.308212')
    # y_spawn_arg = DeclareLaunchArgument('y_spawn', default_value='-11.273542')
    # z_spawn_arg = DeclareLaunchArgument('z_spawn', default_value='0.680563')
